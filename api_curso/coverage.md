...
Las pruebas de integración y funcionales se encuentran en [tests/Feature/ApiCoverageTest.php](tests/Feature/ApiCoverageTest.php). Este archivo evalúa los endpoints HTTP, controladores, validaciones de requests, reglas de negocio y relaciones de Eloquent de la API. Con estas pruebas se alcanza un **78.23% de cobertura en toda la aplicación (líneas)**.
...
| :--- | :---: | :---: | :--- |
| **Controladores (Controllers)** | **77.78%** | Completado | 14 / 18 clases evaluadas |
| **Modelos (Models)** | **77.78%** | Completado | 14 / 18 clases evaluadas |
| **Líneas de Código (Lines)** | **78.23%** | Completado | 496 / 634 líneas evaluadas en total |
| **Métodos (Methods)** | **77.36%** | Completado | 41 / 53 métodos evaluados |
| **Pruebas Funcionales y Unitarias** | **63 / 63** | Aprobado | 147 aserciones ejecutadas sin errores |

### Desglose por Controlador (100% de Cobertura)
* App\Http\Controllers\AuthController - 100.00% (53/53 líneas)
...
```bash
docker exec -e XDEBUG_MODE=coverage bodega_api vendor/bin/phpunit --coverage-text --coverage-html public/coverage
```
...
2. `vendor/bin/phpunit`: Corre todas las pruebas funcionales y unitarias del proyecto.
...
La suite de pruebas en [tests/Feature/ApiCoverageTest.php](tests/Feature/ApiCoverageTest.php) cubre los siguientes flujos transaccionales y de validación:
...
Para lograr un tiempo de ejecución rápido (menos de 9 segundos para 64 pruebas funcionales) sin afectar los datos del entorno de desarrollo o staging en MySQL, el archivo [phpunit.xml](phpunit.xml) configura una base de datos temporal en memoria utilizando SQLite:
