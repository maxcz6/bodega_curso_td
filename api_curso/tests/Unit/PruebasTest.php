...
class PruebasTest extends TestCase
{
    ...
    /**
     * [UNITARIO] Cobertura exhaustiva de relaciones de Modelos y métodos protegidos.
     * @test
     */
/*
    public function test_51_unitario_cobertura_modelos_y_relaciones(): void
    {
        $categoria = Categoria::create(['nombre' => 'Lácteos']);
        $producto = Producto::create([
            'codigo_barras' => '7751234567890',
            'nombre' => 'Leche Gloria',
            'descripcion' => 'Lata 400g',
            'stock_actual' => 50,
            'stock_minimo' => 10,
            'precio_compra' => 3.50,
            'precio_venta' => 4.20,
        ...
        $this->assertInstanceOf(\Illuminate\Database\Eloquent\Relations\HasMany::class, $producto->detallesVenta());
        ...
        $this->assertInstanceOf(\Illuminate\Database\Eloquent\Relations\BelongsTo::class, $venta->tipoComprobante());
        $this->assertInstanceOf(\Illuminate\Database\Eloquent\Relations\BelongsTo::class, $venta->usuario());
        $this->assertInstanceOf(\Illuminate\Database\Eloquent\Relations\HasMany::class, $venta->detalles());

        $this->assertInstanceOf(\Illuminate\Database\Eloquent\Relations\BelongsTo::class, $detalle->venta());
        $this->assertInstanceOf(\Illuminate\Database\Eloquent\Relations\BelongsTo::class, $detalle->producto());

        $this->assertInstanceOf(\Illuminate\Database\Eloquent\Relations\BelongsTo::class, $movimiento->producto());
        $this->assertInstanceOf(\Illuminate\Database\Eloquent\Relations\BelongsTo::class, $movimiento->motivo());
        $this->assertInstanceOf(\Illuminate\Database\Eloquent\Relations\BelongsTo::class, $movimiento->usuario());

        $tipo = TipoComprobante::find(1);
        $this->assertInstanceOf(\Illuminate\Database\Eloquent\Relations\HasMany::class, $tipo->ventas());

        $motivo = MotivoMovimiento::find(1);
        $this->assertInstanceOf(\Illuminate\Database\Eloquent\Relations\HasMany::class, $motivo->movimientos());
        ...
        $refMotivo->invoke($motivo);
    }
*/

    /**
     * [UNITARIO] Cobertura exhaustiva de endpoints de Controladores en la suite unitaria.
     * @test
     */
/*
    public function test_52_unitario_cobertura_controladores_completa(): void
    {
        $loginRes = $this->postJson('/api/auth/login', ['username' => 'admin', 'password' => '123456'])->assertStatus(200);
        $token = $loginRes->json('access_token');

        $this->postJson('/api/auth/login', ['username' => 'admin', 'password' => 'wrong'])->assertStatus(401);
        $this->postJson('/api/auth/login', ['username' => 'no_existe', 'password' => 'wrong'])->assertStatus(404);
        ...
        $this->postJson('/api/auth/login', ['username' => 'user_inactivo', 'password' => '123456'])->assertStatus(403);
        ...
        $cat = $this->postJson('/api/categorias', ['nombre' => 'Abarrotes'])->assertStatus(201)->json('data');
        $this->getJson('/api/categorias/' . $cat['id_categoria'])->assertStatus(200);
        $this->putJson('/api/categorias/' . $cat['id_categoria'], ['nombre' => 'Abarrotes Edit'])->assertStatus(200);

        $this->getJson('/api/clientes')->assertStatus(200);
        $cli = $this->postJson('/api/clientes', ['nombres' => 'Carlos', 'dni_ruc' => '87654321'])->assertStatus(201)->json('data');
        $this->getJson('/api/clientes/' . $cli['id_cliente'])->assertStatus(200);
        $this->putJson('/api/clientes/' . $cli['id_cliente'], ['nombres' => 'Carlos Edit'])->assertStatus(200);

        $prod = $this->postJson('/api/productos', [
        ...
        $this->getJson('/api/productos/' . $prod['id_producto'])->assertStatus(200);
        $this->getJson('/api/productos/bajo-stock')->assertStatus(200);
        $this->putJson('/api/productos/' . $prod['id_producto'], ['nombre' => 'Arroz Edit'])->assertStatus(200);

        $this->getJson('/api/inventario/movimientos')->assertStatus(200);
        ...
        $this->postJson('/api/tipo-comprobantes', ['nombre' => 'Nota de Crédito'])->assertStatus(201);
        ...
        $this->getJson('/api/ventas/' . $venta['id_venta'])->assertStatus(200);

        $this->deleteJson('/api/clientes/' . $cli['id_cliente'])->assertStatus(400);
        $this->deleteJson('/api/categorias/' . $cat['id_categoria'])->assertStatus(400);
        $this->deleteJson('/api/productos/' . $prod['id_producto'])->assertStatus(200);
